...
float CBlackBoard::getFirstEnteredNest()
{
	if (m_firstEnteredNest != -1)
	{
		float firstEnteredNest = (2400.0 - m_firstEnteredNest) / 2400.0;
		//if (robotID != -1) 
		//std::cout << " " << m_firstEnteredNest << " ";// << std::endl;
		return firstEnteredNest;
	}
	else
	{
		return 0.0;
...
{
	if (m_firstDetectedFood != -1)
	{
		float firstDetectedFood = (2400.0 - m_firstDetectedFood) / 2400.0;
		//if (robotID != -1) 
		//std::cout << " " << m_firstDetectedFood << " ";// << std::endl;
		return firstDetectedFood;
	}
	else
	{
		return 0.0;
...
{
	//std::cout << "total density: " << m_totalDensity << std::endl;
...
void CBlackBoard::setFirstDensityChange(int count)
{
	if (m_firstDensityChange == -1)
	{
		float density = 0.0;
		for (float d : m_densityVector)
		{
			density += d;
		}
		density /= m_densityVector.size();
		
		if (density - m_initialDensity > 0.1)
		{
			m_firstDensityChange = count;
			//std::cout << "m_firstDensityChange = " << std::to_string(count) << std::endl;
		}
	}
}
...
void CBlackBoard::saveDistanceNearestRobot(float distance, int robotID)
{
	distance = 100 - distance;
	m_totalDistanceNearestRobot.push_back(distance);
	//if (robotID != -1) std::cout << distance << std::endl;
}

void CBlackBoard::saveNumNeighbours(int neighbours, int robotID)
{
	if (m_totalNumNeighbours.size() == 0)
	{
		m_initialNumRobots = neighbours;
	}
	
	m_totalNumNeighbours.push_back(neighbours);
	//if (robotID != -1) std::cout << neighbours << std::endl;
}

float CBlackBoard::getAvgDistanceNearestRobot()
{
	float distance = 0.0;
	for (float d : m_totalDistanceNearestRobot)
	{
		distance += d;
	}
	
	return distance / m_totalDistanceNearestRobot.size();
}

float CBlackBoard::getStdDevDistanceNearestRobot()
{
	float mean = getAvgDistanceNearestRobot();
	float deviation = 0.0;
	
	for (float d : m_totalDistanceNearestRobot)
	{
		deviation += (d - mean) * (d - mean);
	}
	
	deviation /= m_totalDistanceNearestRobot.size();
	return sqrt(deviation);
}

float CBlackBoard::getAvgNumNeighbours()
{
	int neighbours = 0;
	for (int n : m_totalNumNeighbours)
	{
		neighbours += n;
	}
	
	float avgNeighbours = static_cast<float>(neighbours) / static_cast<float>(m_totalNumNeighbours.size());
	
	return avgNeighbours;
}

float CBlackBoard::getStdDevNumNeighbours()
{
	float mean = getAvgNumNeighbours();
	float deviation = 0.0;
	
	for (int n : m_totalNumNeighbours)
	{
		float neighbours = static_cast<float>(n);
		deviation += (neighbours - mean) * (neighbours - mean);
	}
	
	deviation /= m_totalNumNeighbours.size();
	return sqrt(deviation);
}
...
//{
	//if (m_motors == 1 || m_motors == 2 || m_motors == 3)
	//{
		//m_movement++;
	//}
	//if (m_motors == 4 || m_motors == 5 || m_motors == 6)
	//{
		//m_movement--;
	//}
//}
